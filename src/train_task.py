...
from config import *
from model import LM,LM2,RoyNet


# wandb.init(project = "Natural Language Understanding ", entity = "roy3" , run = "try 1")

def train2(train_dataset_filepath, val_dataset_filepath, epochs, model_path = NET2_MODEL_PATH, batch_size=BATCH_SIZE, learning_rate=LEARNING_RATE, model_saved=False):
    ...
    train_dataset = Dataset2(train_dataset_filepath)
    val_dataset = Dataset2(val_dataset_filepath)
...
# begin training
    ...
    for epoch in range(epochs):
        ...
        for i, example in enumerate(train_dataloader):
            ...
            #input vector would have shape (N,L,D) but we want (L,N,D)
            ...
            train_count += predictions.shape[0]
        ...
        if (epoch + 1) % 1 == 0:
            ...
            torch.save(model.state_dict(), NET2_MODEL_PATH)
